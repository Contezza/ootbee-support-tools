...
function main() {
  ...
  if (!jobId) {
    ...
    model.stringifiedJob = "jobId must be specified";
    return;
  }

  var job = batchExecuter.getJobSummary(jobId);

  if (!job || Object.keys(job).length === 0) {
    status.code = 404;
    status.message = "Job not found";
    status.redirect = true;
    model.stringifiedJob = "Job not found";
    return;
  }

  model.stringifiedJob = JSON.stringify(job);
}
